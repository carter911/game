...
class Grid
{
    ...
    public function column($prop,$label="")
    {
        ...
        if(self::showFormByField($prop,1)){
            $grid = $grid->setAddDisplay(false);
        }
        $this->column[] = $grid;
        return $grid;
    }

    protected static $addDisable = ['id','_time','_at',];
    protected static $addReadOnly = ['id','_time','_at',];
    protected static $editDisable = ['id','_time','_at',];
    protected static $editReadOnly = ['id','_time','_at',];
    public static function showFormByField($prop='',$type)
    {
        $flag = false;
        if($type ==1){
            foreach (self::$addDisable as $key => $value){
                if(strpos(strtolower($prop),$value) != false){
                    $flag =  true; break;
                }
            }
        }
